...
<title>交流<%=newJcpeizhi.getSjduochuBieming()%></title>
...
var url;
	...
	function openSjduochuAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加交流<%=newJcpeizhi.getSjduochuBieming()%>");
		url="../addSjduochu";
	}
	...
	function openSjduochuModifyDialog(){
		...
		$("#dlg").dialog("open").dialog("setTitle","编辑交流<%=newJcpeizhi.getSjduochuBieming()%>");
		$("#fm").form("load",row);
	...
	function daoruSjduochus(){
		$("#daoru").dialog("open").dialog("setTitle","导入交流<%=newJcpeizhi.getSjduochuBieming()%>");
	...
	function shangchuanSjduochu(){
		...
		var row=selectedRows[0];
		$("#shangchuan").dialog("open").dialog("setTitle","上传交流<%=newJcpeizhi.getSjduochuBieming()%>");
	...
	function duoxiangSjduochu0(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangSjduochu",{delIds:ids,sjduochuType:0},function(result){
					if(result.success){
	...
	function duoxiangSjduochu1(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangSjduochu",{delIds:ids,sjduochuType:1},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功处理<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
	...
	function chakanNeirong(sjduochuId, row) {
		return '<a style="color:red;"target="_blank" href="<%=basePath%>neirong/sjduochuneirong.jsp?sjduochuId=' + sjduochuId + '">查看' + '</a>';  
	}
</script>
</head>
<body style="margin: 5px;">
<!--startprint-->
	<table id="dg" title="交流<%=newJcpeizhi.getSjduochuBieming()%>" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSjduochus" fit="true" rownumbers="true" toolbar="#tb" nowrap="false">
		<thead>
			<tr>
				...
				<th field="sjduochuImg" width="10" formatter="formatChakan">缩略图</th>
				<th field="sjduochuDate" width="20" formatter="datetostr" hidden="true">时间</th>
			</tr>
		</thead>
	</table>
