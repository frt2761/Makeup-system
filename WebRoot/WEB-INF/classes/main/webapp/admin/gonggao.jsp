...
var url;
	...
	function chakanNeirong(gonggaoId, row) {
		return '<a style="color:red;"target="_blank" href="<%=basePath%>neirong/gonggaoneirong.jsp?gonggaoId=' + gonggaoId + '">查看' + '</a>';  
	}
</script>
</head>
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getGonggaoBieming()%>信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getGonggaos" fit="true" rownumbers="true" toolbar="#tb" nowrap="false">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="gonggaoId" width="10" formatter="chakanNeirong">查看</th>
				<th field="gonggaoName" width="80">标题</th>
				<th field="ggtypeId" width="10" hidden="true"><%=newJcpeizhi.getGgtypeBieming()%>ID</th>
				<th field="ggtypeName" width="20"><%=newJcpeizhi.getGgtypeBieming()%></th>
				<th field="gonggaoImg" width="20" formatter="formatChakan">缩略图</th>
				<th field="gonggaoDate" width="20" formatter="datetostr" hidden="true">时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb">
		<div>
			...
			<a href="javascript:shangchuanGonggao()" class="easyui-linkbutton" iconCls="icon-add" plain="true">上传缩略图</a>
		</div>
	</div>
<!--endprint-->
		...
		<form name="fm" id="fm" method="post">
			<table>
				<tr>
					...
					<td><%=newJcpeizhi.getGgtypeBieming()%>：</td>
