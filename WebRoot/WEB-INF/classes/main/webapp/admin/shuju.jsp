...
var url;
	...
	function duoxiangShuju0(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangShuju",{delIds:ids,shujuType:0},function(result){
					if(result.success){
	...
	function duoxiangShuju1(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangShuju",{delIds:ids,shujuType:1},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功处理<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
	...
	function xianshiTupian(shujuImg, row) {
		if(shujuImg){
			return '<img src="../' + shujuImg + '" width="100" height="100">'; 
		}else {
			return "未上传";
		}
	}
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getShujuBieming()%>信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getShujus" fit="true" rownumbers="true" toolbar="#tb" nowrap="false">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="shujuId" width="10" formatter="chakanNeirong">查看</th>
				<th field="shujuImg" width="25" formatter="xianshiTupian">缩略图</th>
				<th field="shujuName" width="40">名称</th>
				<th field="shujuDouble" width="10">售价</th>
				<th field="shujuZong" width="10">数量</th>
				<th field="shujuMark1" width="20">产地</th>
				<th field="shujuMark2" width="40">特点</th>
				<th field="shujuMark3" width="40">备注</th>
...
<!--endprint-->
	<div id="dlg" class="easyui-dialog" style="width: 850px;height: 480px;padding: 10px 20px"
		...
		<form id="fm" method="post">
			<table cellspacing="5px;">
				...
				<tr>
					...
					<td><%=newJcpeizhi.getBuyuanBieming()%>：</td>
					<td><input class="easyui-combobox" id="buyuanIds" name="buyuanIds"  data-options="multiple:true,panelHeight:'auto',editable:false,valueField:'buyuanId',textField:'buyuanName',url:'../buyuanComboList'"/></td>
				</tr>
				<tr>
					<td>数量：</td>
