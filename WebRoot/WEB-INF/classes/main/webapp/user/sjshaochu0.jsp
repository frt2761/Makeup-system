...
var userId = <%=userId%>;
	...
	function quxiaoSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要执行吗？",function(r){
			if(r){
				$.post(url,{sjshaochuType:4},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
	...
	function duoxiangSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangSjshaochu",{delIds:ids,sjshaochuType:1},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功处理<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert('系统提示','<font color=red>'+selectedRows[result.errorIndex].sjshaochuName+'</font>'+result.errorMsg);
					}
	...
	function fukuanSjshaochu(){
		...
		$.messager.confirm("系统提示","您确认要处理这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../fukuanSjshaochu",{delIds:ids,sjshaochuType:1,userId:userId},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
	...
	function erweiSjshaochu(){
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length==0){
			$.messager.alert("系统提示","请选择要付款的数据！");
			return;
		}
		var strIds=[];
		for(var i=0;i<selectedRows.length;i++){
		...
		var ids=strIds.join(",");
		$("#sjfukuan").dialog("open").dialog("setTitle","付款<%=newJcpeizhi.getShujuBieming()%><%=newJcpeizhi.getSjshaochuBieming()%>");
		sjfukuanurl="../erweiSjshaochu?sjshaochuType=1&delIds="+ids+"&userId="+userId;
	}
	
	function closeSjfukuanSjshaochu(){
		$("#sjfukuan").dialog("close");
		resetValue();
	}
	
	function saveSjfukuanSjshaochu(){
		$("#shchfm").form("submit",{
			url:sjfukuanurl,
			onSubmit:function(){
				return $(this).form("validate");
	...
	function compute() {//计算函数
		...
		var sjshaochuDouble1total = 0//计算sjshaochuDouble1的总和
		for (var i = 0; i < rows.length; i++) {
			sjshaochuDouble1total += parseFloat(rows[i]['sjshaochuDouble1']);
		}
		$('#dg').datagrid('appendRow', { shujuName: '<b>统计：</b>',sjshaochuDouble1:sjshaochuDouble1total});
	}
	
	function xianshiTupian(shujuImg, row) {
		if(shujuImg){
			return '<img src="../' + shujuImg + '" width="150" height="150">'; 
		}else {
			return "未上传";
		}
	}
...
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getShujuBieming()%><%=newJcpeizhi.getSjshaochuBieming()%>" class="easyui-datagrid" fitColumns="true"
	 pagination="true" data-options="url:'../getSjshaochus?sjshaochuType=0&userId=<%=userId %>',method:'post',onLoadSuccess:compute" fit="true" rownumbers="true" toolbar="#tb" nowrap="false">
		<thead>
			<tr>
				...
				<th field="sjshaochuImg" width="23" formatter="xianshiTupian">缩略图</th>
	...
	<div id="tb">
		<div>
			<a href="javascript:openSjshaochuModifyDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a>
			<a href="javascript:fukuanSjshaochu()" class="easyui-linkbutton" iconCls="icon-add" plain="true">付款</a>
			<a href="javascript:deleteSjshaochu()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
		</div>
		<div>
...
<!--付款-->	
	<div id="sjfukuan" class="easyui-dialog" style="width: 420px;height: 460px;padding: 10px 20px"
		closed="true" buttons="#sjfukuan-buttons">
		<form id="shchfm" method="post">
			<table cellspacing="5px;">
				<tr>
					<td><img src="<%=basePath %>file/erweima.jpg" width="350" height="350" /></td>
				</tr>
			</table>
		</form>
	</div>
	
	<div id="sjfukuan-buttons">
		<a href="javascript:saveSjfukuanSjshaochu()" class="easyui-linkbutton" iconCls="icon-ok">付款</a>
		<a href="javascript:closeSjfukuanSjshaochu()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
